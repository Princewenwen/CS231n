...
def rnn_backward(dh, cache):
  ...
  for i in xrange(T-1, -1, -1):
    dx[: , i, :], dprev_h_step, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:, i, :] + dprev_h_step, cache.pop())  ## +dprev_h_step ??
    dh0 = dprev_h_step
    dWx += dWx_step
    dWh += dWh_step
    db += db_step
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  ai, af, ao, ag = np.array_split(a, 4)
  ai = a[:, :H]
  af = a[:, H:2*H]
  ao = a[:, 2*H:3*H]
  ag = a[:, 3*H:]
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  da = np.hstack((dai, daf, dao, dag))  ## np.hstack
  dx = da.dot(Wx.T)
  dWx = (x.T).dot(da)
  dprev_h = da.dot(Wh.T)
  dWh = (prev_h.T).dot(da)
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for i in xrange(T):
    h[:, i, :], cell, sub_cache = lstm_step_forward(x[:, i, :], hidden, cell, Wx, Wh, b)
    hidden = h[:, i, :]
...
def lstm_backward(dh, cache):
  ...
  db = np.zeros((4*H))
  for i in xrange(T-1, -1, -1):
    dx_step, dprev_h, dprev_c_step, dWx_step, dWh_step, db_step = lstm_step_backward(dh[:, i, :] + dprev_h, dprev_c_step, cache.pop())
    dWx += dWx_step
    dWh += dWh_step
    db += db_step
    dx[:, i, :] += dx_step
