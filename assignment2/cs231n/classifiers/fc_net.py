...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dout, ddropout, dbn, dh = {}, {}, {}, {}
    t = self.num_layers - 1
    dout[t], grads['W'+str(t+1)], grads['b'+str(t+1)] = affine_backward(dscores, cache)
    #dW+str(nl) += W+str(nl)
    #grads['W'+str(nl)] = dW+str(nl)
    #grads['b'+str(nl)] = db+str(nl)
    #i = nl-1
    for i in xrange(t):
      if self.use_batchnorm:
        if self.use_dropout:
          ddropout[t-i-1] = dropout_backward(dout[t-i], cache4[t-i-1])
          dbn[t-i-1] = relu_backward(ddropout[t-i-1], cache3[t-i-1])
        else:
          dbn[t-i-1] = relu_backward(dout[t-i], cache3[t-i-1])
        dh[t-i-1], grads['gamma'+str(t-i)], grads['beta'+str(t-i)] = batchnorm_backward(dbn[t-1-i],
                                                                                        cache2[t-i-1])
        dout[t-i-1], grads['W'+str(t-i)], grads['b'+str(t-i)] = affine_backward(dh[t-i-1],
                                                                                cache1[t-i-1])
      else:
        if self.use_dropout:
          ddropout[t-i-1] = dropout_backward(dout[t-i], cache4[t-i-1])
          dout[t-i-1],grads['W'+str(t-i)],grads['b'+str(t-i)] = affine_relu_backward(ddropout[t-i-1],
                                                                                   cache3[t-i-1])
        else:
          dout[t-i-1],grads['W'+str(t-i)],grads['b'+str(t-i)] = affine_relu_backward(dout[t-i],
                                                                                   cache3[t-i-1])
    for i in xrange(self.num_layers):
