...
def batchnorm_backward(dout, cache):
  ...
  dgamma = np.sum(x_normalized * dout, axis = 0, keepdims = True)
  ...
  dx = dx_normalized * sample_std_inv + 2.0 * dsample_var * x_mu / N + dsample_mean/N
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
    ...
    for f in xrange(F):
      for i in xrange(H_new):
        for j in xrange(W_new):
          #x_win = x_padded[n, :, i*s:i*s+HH, j*s:j*s+WW]
          #out[n, f, i, j] = np.sum(x_win * w[f]) + b[f]
          out[n, f, i, j] = np.sum(x_padded[n, :, i*s:HH+i*s, j*s:WW+j*s] * w[f]) + b[f]
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def conv_backward_naive(dout, cache):
  ...
  x_padded = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant')
  dx_padded = np.pad(x_padded, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant')
  ...
  for n in xrange(N):
    for f in xrange(F):
      for i in xrange(H_new):
        for j in xrange(W_new):
          x_padded_win = x_padded[n, :, i*s:i*s+HH, j*s:j*s+WW]
          db[f] += dout[n, f, i, j]
          dw[f] += dout[n, f, i, j] * x_padded_win
    ...
    for c in xrange(C):
      for i in xrange(H_new):
        for j in xrange(W_new):
          x_win = x[n, c, i*s:i*s+HH, j*s:j*s+WW]
          m = np.max(x_win)
          dx[n, c, i*s:i*s+HH, j*s:j*s+WW] += dout[n, c, i, j] * (x_win == m)
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
